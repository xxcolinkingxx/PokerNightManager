  ...
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      className={cn(
        ...
        side === "left" &&
          "inset-y-0 left-0 h-full w-3/4 rounded-r-3xl data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm",
        side === "right" &&
          "inset-y-0 right-0 h-full w-3/4 rounded-l-3xl data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm",
        className,
      )}
      {...props}
    >
      ...
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 opacity-70 transition-opacity hover:bg-white/5 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-gold/40 disabled:pointer-events-none">
        <X className="h-4 w-4" />
...
const SheetHeader = ({
  ...
  <div
    className={cn("flex flex-col space-y-2 px-6 pt-2 text-center sm:text-left", className)}
    {...props}
  />
);
